...
第一阶段是获取像素p在参考图像I<sub>ref</sub>中深度值的假设集。 在计算I<sub>ref</sub>和相邻图像的图像一致性曲线后，深度范围内的曲线图像，保留从所有曲线中得分最高的K个曲线的峰值{di（p）|i∈[1，K]}，图像一致性函数采用NCC算法。 如前所述，该算法的另一个关键特征是包含未知状态U，当没有足够的选择证据时。 因此对于每个像素，它们形成深度标签集{{d<sub>i<s/sub>(p)},U}。

#### MRF Optimization
深度标签分配被公式化为MRF，其中每个像素具有一组最多（K + 1）个标签。 第一个K标签，如果在标签提取阶段找到的峰数不足，则更少，对应于照片一致性函数中的峰值并且具有相关深度di（p）和得分C（p，di（p））。 最终的状态是如前所述的未知状态U.
一元成本很简单。 我们希望以较低的匹配分数惩罚局部最大值，因为它们更可能对应不正确的匹配。 他们采用逆指数函数
将此分数映射到正成本[190]，而常数惩罚ΦU被强制用于未知状态以避免分配深度值具有较差的照片一致性，并且没有来自相邻像素的成对支持：

***********公式***********
成对术语强制进行空间正则化。 那里有两个标签类型（深度和未知状态）和成对成本在以下4（= 2×2）个案例中定义：

***************公式*******************

在上面的第一种情况中，两个标签都是深度值，即成本只需测量（3.6）中的差异量。请注意差异通过深度值的平均值进行归一化，以使其与规模相关性降低。在第二和第三种情况下，其中之一标签是未知状态，不断处罚是为了防止频繁切换深度标签和未知状态。在里面最后一种情况，两个标签都是同一个未知状态，罚款是设置为0以支持空间一致性。遗憾的是，成对成本在这个公式中不是子模块，因为每个像素的深度标签是独立提取的，并且第i个标签的含义对于不同而言是任意不同的像素。例如，Ψ（di（p），di（q））在标准MRF公式（3.6）中为0，因为di（p）和di（q）是像素无关的并且对应于相同的深度值。但是，事实并非如此公式。因此，alpha扩展不适用，而是消息传递算法，如循环信念传播（LBP）[204]和可以使用树重新加权消息传递（TRW）[194]，这是MRF的其他流行的优化技术。特别是，TRW已成功应用于解决许多计算机视觉问题包括深度图重建[123,179]，并用于他们的工作。
图3.8说明了遮挡时十个连续像素的光稳定性曲线和局部最大值的位置边界。请注意，未知标签分配给的像素遮挡边界（从顶部开始的第六个像素），其中强制执行空间正则化以分配正确的深度标签，即使在其中曲线的全局最大值对应于假深度（第四像素从一开始）。图3.9列出了更多的实验结果一些中间重建用于评估。如图所示，单个深度图包含在像素处的孔分配了未知状态标签，并且未分配表面区域在参考图像中可见。然而，重要的是仅以高置信度重建区域以最小化噪声的存在在以下合并步骤中。图3.9g说明了该模型在仅两个深度图的叠加中变得接近完整。

#### 3.1.5 More Depthmap Reconstruction Algorithms

已经提出了许多深度图重建算法过去除了前面几节中描述的内容。 这个部分介绍了一些更重要的算法和技术文献

##### Real-Time Plane Sweeping Depthmap Reconstruction
深度图重建不是计算上廉价的操作
需要在每个像素和每个假设深度的多个图像上评估照片一致性函数。然而，盖洛普，弗拉姆，莫尔多海，杨和波勒菲斯证明了这一点
聪明地使用GPU可以实现实时[76]。该算法被称为“平面扫描立体声”，因为它扫描了一个家庭
场景中的平行平面，通过平面将图像投影到平面上
单应性，然后评估每个平面上的照片一致性值。
每个像素的深度值由“赢者通吃”策略选择，
算法中有两个关键特征。
